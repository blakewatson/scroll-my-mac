...
**In `handleMouseDown`:** After entering hold-and-decide (`pendingMouseDown = true`), if `holdToPassthroughEnabled` is true, start a GCD timer on the main queue with delay `holdToPassthroughDelay`. When the timer fires:
1. Cancel the timer
